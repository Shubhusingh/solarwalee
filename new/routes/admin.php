...
Route::group(['middleware'=>'permissions:Loan Management'],function(){
    ...
    Route::get('/lead-all',[LoanController::class,'leadall'])->name('admin.lead.index');
  
  

  });
 
  Route::get('/loan/update',[LoanController::class,'status_update']);
  Route::post('/loan/update/store',[LoanController::class,'update_status']);
  Route::post('/loan/assign',[LoanController::class,'assignlead']);
  

  Route::get('/loan/assign',[LoanController::class,'assign_update']);
  Route::get('loan/approve/{id}',[LoanController::class,'loanapprove']);

  Route::get('/lead/interest', [LoanController::class, 'interest']);

  Route::post('/lead/interest/store', [LoanController::class, 'leadstatus1']);
      
       Route::get('/lead/tracking', [LoanController::class, 'leadtracking']);
        Route::get('/lead/pickup', [LoanController::class, 'pickupstatus']);
        
         Route::post('/lead/interest/store2', [LoanController::class, 'leadstatus2']);
         Route::get('/lead/follow', [LoanController::class, 'follow']);
         
         
          Route::post('/lead/interest/store4', [LoanController::class, 'leadstatus4']);
          
          Route::post('/lead/lead_status', [LoanController::class, 'statusupdate']);
           Route::get('/lead/assgin_lead', [LoanController::class, 'lead_assign']);
           
            Route::post('/lead/lead/assign', [LoanController::class, 'statusassign']);

 

    // Route::get('/lead',[LeadController::class,'index'])->name('admin.lead.index');

    //     Route::get('/Not_Eligible',[LeadController::class,'Not_Eligible'])->name('admin.Not.Eligible');
    //      Route::get('/less_salery',[LeadController::class,'lesssalery'])->name('admin.less.salery');
         
    //        Route::get('/Docs_Received',[LeadController::class,'docrevi'])->name('admin.Docs.Received');
           
    //         Route::get('/Incomplete_Docs',[LeadController::class,'Incomplete_Docs'])->name('admin.Incomplete.Docs');
            
    //         Route::get('/paydayloan',[LeadController::class,'paydayloan'])->name('admin.pay.loan');
    
    //  Route::post('/statusupdate',[LeadController::class,'statusupdate'])->name('status.update');
...
Route::middleware('admin', 'adminPermission')->group(function () {
    ...
    Route::name('loan.')->prefix('loan')->controller('LoanController')->group(function () {
        ...
        Route::get('freshlead', 'runningLoans')->name('running');
        Route::get('pending', 'pendingLoans')->name('pending');
        Route::get('rejected', 'rejectedLoans')->name('rejected');
        Route::get('lesssalery', 'paidLoans')->name('paid');
        Route::get('NotEligible', 'Not_Eligible')->name('noteligible');
        Route::get('interestlead', 'dueInstallment')->name('due');
        Route::post('approve/{id}', 'approve')->name('approve');
