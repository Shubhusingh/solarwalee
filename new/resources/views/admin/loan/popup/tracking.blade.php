...
@foreach ($tracking as $value)
	...
	<div class="timeline__item">
		<div class="timeline__item-header">
			...
			<p style="margin-left: 60px;margin-top: -17px">{{$value->date ?? ''}}    {{$value->time ?? ''}}</p>
				
		<p style="margin-left: 60px;margin-top: -17px">{{$value->remarks ?? ''}}</p>
		
		@php
		$user=DB::table('admins')->where('id',$value->user_id)->first();
		@endphp
